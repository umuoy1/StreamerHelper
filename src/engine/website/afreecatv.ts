...
export function main(url: string) {
  return new Promise(async (resolve, reject) => {
    headers['Referer'] = url;
    const m = url.match(/com\/(\S*)\//g);
    let bid;
    if (m) {
      bid = m[0].split('/')[1];
    } else {
      reject('AFREECATV=>Room not exists!');
      return;
    }
    try {
      ...
      if (channel.BPWD === 'Y') {
        ...
        reject('AFREECATV=>No match results:Maybe the roomid is error,or this room is not open!');
      ...
      for (const qkey of QUALITYS) {
        try {
          const hls_stream = await getHlsStream(broadcast, bid, qkey, cdn, rmd);
          if (hls_stream) {
            resolve(hls_stream);
            return;
          }
