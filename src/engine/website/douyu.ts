...
export function main(url: string) {
  return new Promise((resolve, reject) => {
    ...
    axios
      .get(`https://www.douyu.com/wgapi/live/liveweb/getRoomLoopInfo?rid=${rid}`)
      .then((response: any) => {
        const roomloop: any = response.data;
        if (roomloop['data']['vid'] != '') {
          reject('DOUYU=>It is video now.');
        }
        axios
