...
export function main(url: string) {
  return new Promise((resolve, reject) => {
    if (url.indexOf('user') != -1) {
      ...
      axios
        .get(
          `https://webh.huajiao.com/User/getUserFeeds?_callback=padding&uid=${uid}&fmt=jsonp&_=${Date.now()}`
        )
        .then((response: any) => {
          ...
          if (!feedInfo?.['relay']) {
            reject('HUAJIAO=>No match results:Maybe the roomid is error,or this room is not open!');
          }
      ...
      axios
        .get(`https://www.huajiao.com/l/${rid}`)
        .then((response: any) => {
          ...
          global.app.logger.warn(
            `HUAJIAO => roomUrl: ${url} 只能录制当次直播，请更换 roomUrl 为 https://www.huajiao.com/user/${uid}`
          );
        ...
        .catch((error: any) => {
          global.app.logger.warn(
            `HUAJIAO => roomUrl: ${url} 只能录制当次直播，请更换 roomUrl 为 https://www.huajiao.com/user/xxxx`
          );
          reject(error);
        });
    } else {
