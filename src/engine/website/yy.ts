...
export function main(url: string) {
  return new Promise((resolve, reject) => {
    ...
    if (!rid) {
      reject('YY=>No match results:Maybe the roomid is error,or this room is not open!');
    }
    ...
    const config: any = {
      ...
      headers: {
        ...
        'User-Agent':
          'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${Math.floor(Math.random() * (120 - 100 + 1)) + 100}.0.0.0 Safari/537.36',
      },
      data,
    };
    axios(config)
