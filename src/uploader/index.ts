...
export class uploader {
  ...
  constructor(recorderTask: RecorderTask) {
    ...
    this.cover = '';
    this.desc =
      recorderTask.streamerInfo.desc ||
      'Powered By StreamerHelper. https://github.com/ZhangMingZhao1/StreamerHelper';
    this.no_reprint = 0;
    this.open_elec = 1;
    this.source =
      recorderTask.streamerInfo.source ||
      `${recorderTask.recorderName} 直播间: ${recorderTask.streamerInfo.roomUrl}`;
    this.tags = recorderTask.streamerInfo.tags;
    this.tid = recorderTask.streamerInfo.tid;
    this.title =
      `${recorderTask.recorderName} ${recorderTask.timeV} 录播` ||
      this.renderTitle(recorderTask.streamerInfo.templateTitle || '', {
        time: recorderTask.timeV,
        name: recorderTask.recorderName,
      });
    this.dynamic =
      recorderTask.streamerInfo.dynamic ||
      `${recorderTask.recorderName} 直播间: ${recorderTask.streamerInfo.roomUrl}`;
  ...
  upload = async () => {
    return new Promise<void>(async (resolve, reject) => {
      if (!this.uploadLocalFile)
        return this.logger.info(
          `User config => ${this.recorderName} uploadLocalFile ${this.uploadLocalFile}. Upload give up...`
        );
      ...
      try {
        ...
        if (uploadStatus.get(this.dirName) === 1)
          return this.logger.error(
            `目录 ${this.dirName} 正在上传中，避免重复上传，取消此次上传任务`
          );
        ...
        this.logger.info(
          `Upload videoParts END, remoteVideos: ${JSON.stringify(remoteVideos, null, 2)}`
        );
  ...
  getLocalVideos(path: string): localVideoPart[] {
    ...
    fs.readdirSync(path).forEach((shortPath: string) => {
      ...
      if (fileSize < videoPartLimitSize) {
        this.logger.info(
          `${chalk.red('放弃该分P上传')} ${fullPath}, 文件大小 ${Math.round(fileSize / 1024 / 1024)}M, 限制${this.videoPartLimitSizeInput}M`
        );
      } else if (this.succeedUploaded?.find(item => item.localFilePath === fullPath)) {
        this.logger.info(`该分P已经上传成功，跳过 ${fullPath}`);
      } else {
        if (
          this.isUploadFail &&
          this.failUpload?.path === fullPath &&
          (this?.failUpload?.deadline ?? 0) > (this?.failUpload?.uploadStartTime ?? 0) + 7200
        ) {
      ...
      for (let i = 0; i < localVideoParts.length; i++) {
        ...
        try {
          ...
          if (isFailed) {
            ...
            this.logger.info(
              `Get Fail Video Upload Status serverFileName ${serverFileName} uploadUrl${uploadUrl} completeUploadUrl${completeUploadUrl}`
            );
          } else {
          ...
          this.logger.debug(
            `path ${localFilePath} serverFileName ${serverFileName} uploadUrl${uploadUrl} completeUploadUrl${completeUploadUrl}`
          );
          const videoOnServer = await this.uploadVideoPart(
            fileSize,
            localFilePath,
            title,
            desc,
          ...
          if (fs.existsSync(fileStatusPath)) {
            ...
            const result = obj.videoParts.succeedUploaded.find(
  ...
  getPreUploadData = async (): Promise<uploadVideoPartInfo> => {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        const uploadUrl: string = url,
          completeUploadUrl: string = complete,
          serverFileName: string = filename;
        const params = uploadUrl.split('?')[1];
        const { deadline, uploadstart } = querystring.parse(params);

        if (deadline) this.deadline = parseInt(deadline as string);

        if (this.uploadStart) this.uploadStart = parseInt(uploadstart as string);

        this.logger.debug(
          `_getPreUploadData deadline ${this.deadline} uploadStartTime ${this.uploadStart}`
        );
  ...
  uploadVideoPart = async (
    fileSize: number,
    path: string,
    title: string,
    desc: string,
    uploadUrl: any,
    completeUploadUrl: any,
    serverFileName: any,
    ...
    return new Promise<remoteVideoPart>(async (resolve, reject) => {
      ...
      let nowChunk = 0;
      this.logger.info(
        `开始上传 ${path}，文件大小：${fileSize}，分块数量：${chunkNum}， succeedUploa：${successUploadedChunks}`
      );
      ...
      fileStream.on('data', async (chunk: any) => {
        ...
        if (readLength >= chunkSize || totalReadLength === fileSize) {
          ...
          if (nowChunk >= successUploadedChunks) {
            ...
            try {
              this.logger.debug(
                ` nowChunk ${nowChunk} succeedUpload ${successUploadedChunks} uploadUrl ${uploadUrl}, serverFileName ${serverFileName}, path ${path}, chunk.length ${chunk.length}, nowChunk + 1 ${nowChunk + 1}, chunkNum ${chunkNum}`
              );
              ...
              return reject(
                `An error occurred when upload video part: chunk ${nowChunk}/${chunkNum} 分块 ${path} ${err}`
              );
            }
          }
  ...
  uploadChunk = async (
    ...
    return new Promise<void>(async (resolve, reject) => {
      try {
        ...
        this.logger.info(
          `chunk #${chunk_id}/${chunk_total_num} upload ended, returns: ${result.OK}\nPath: ${path}`
        );

        if (result.info !== 'Successful.') {
          reject(
            `Upload Chunk #${chunk_id}/${chunk_total_num} Return Outside Of Expect: ${result.info}`
          );
        }
  ...
  postVideo = async (remoteVideos: remoteVideoPart[]) => {
    return new Promise<void>(async (resolve, reject) => {
      ...
      this.logger.debug(
        `postData ${JSON.stringify(post_data, null, 2)} params${JSON.stringify(params, null, 2)}`
      );
      ...
      try {
        ...
        if (code === 0) {
          this.logger.info(
            `Post End ${code} message ${message} ttl ${ttl} aid ${aid} bvid ${bvid}`
          );
          resolve();
        } else {
          uploadStatus.delete(this.dirName);
          reject(
            `Upload fails, returns:, ${code} message ${message} ttl ${ttl} aid ${aid} bvid ${bvid}`
          );
        }
      } catch (err) {
        uploadStatus.delete(this.dirName);
